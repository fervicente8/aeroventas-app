...
export default function AddAirplane({
  ...
  const pickImage = async (index = 0) => {
    ...
    if (!result.canceled) {
      const newAirplaneImages = [...airplaneImagesUri];
      newAirplaneImages[index] = result.assets[0].uri;

      if (newAirplaneImages[newAirplaneImages.length - 1] !== "") {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/add-buy-plane`, {
        method: "POST",
        body: formData,
        headers: {
      ...
      if (response.ok) {
        setShowAlert({ message: "Se ha agregado la aeronave" });
        setPlanes([...planes, airplane]);
        resetAll();
        ...
        setShowAlert({
          message:
            "Hubo un error al agregar la aeronave. Por favor, intenta de nuevo",
        });
        resetAll();
      }
    } catch (error) {
  ...
  return (
    <ScrollView>
      ...
      >
        <ThemedText style={styles.submit_button_text}>Guardar avión</ThemedText>
      </TouchableOpacity>
    </ScrollView>
  );
}
